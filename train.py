...
def train_one_epoch_ce_mm1(train_dataloader, val_dls,  model, criterion, optimizer, opt, epoch, n_samples, class_embeds):
    ...
    for i ,(d) in bar: 
        ...
        text_mode = torch.zeros(vids.shape[0], class_embeds.shape[2], class_embeds.shape[3]).to(opt.device)

        #print("class embed size", class_embeds.shape)
        for j in range(vids.shape[0]):
            text_mode[j] = class_embeds[0][labels[j]]
        #print("text mode shpae is", text_mode.shape)

        outs = model(vids, text_mode)
        #print(outs.shape)
...
def train_one_epoch_pws(train_dataloader, val_dls,  model, criterion, optimizer, opt, epoch, n_samples, class_embeds):
    ...
    for i ,(d) in bar: 
        ...
        if i % loss_cycle == 0:
            wandb.log({opt.dataset + " " + opt.traintype + " Classification Loss" : loss.item()})
            wandb.log({opt.dataset + " " + opt.traintype + " Train Top-1 Accuracy" : totaltrainacc1 / (i + 1)})
            wandb.log({opt.dataset + " " + opt.traintype + " Train Top-5 Accuracy" : totaltrainacc5 / (i + 1)})
